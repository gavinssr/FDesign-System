...
- AGENTS.md 导航体系建立
- .agent/ 状态管理就绪
- packages/tokens, components, eslint-config, tsconfig 骨架就绪
- ESLint 边界规则可执行
- docs/NORTH_STAR.md 项目定位精华就绪
- docs/ROADMAP.md 全 Phase 路线图就绪
- 四层记忆架构就绪（AGENTS.md + checkpoint + ROADMAP + NORTH_STAR）
- 所有治理文档统一为中文
...
- stage 展示页覆盖所有已实现组件
- 版本管理工具引入（changeset 或类似），支持组件包语义化版本发布
...
- `Text` 在 Phase 2 曾作为高频组件实现，用于完成当时的模式验证与消费层验证
- 在 Phase 5 的排版协议治理中，已确认不再保留 `Text` 作为设计系统通用组件，stage 中改由 `Typography 排版` 基础验证页承接相关展示
- `ListItem` 在 Phase 2 曾作为高频组件实现，用于完成当时的列表展示与消费层验证
- 在 Phase 5 的表单协议治理中，展示类能力已收敛到 `Form` 组件族，`ListItem` 不再作为当前设计系统公共组件继续演进
- `Input / Card / Modal` 在 Phase 2 曾作为高频组件实现，用于完成当时的消费层验证
- 在 Phase 5 的当前组件范围治理中，用户已确认 `Input / Card / Modal` 不再作为当前设计系统公共组件保留；组件本体、公开导出、stage 分支页与导航入口已移除，历史需求若再次出现需重新定义真实范围
...
**scope 说明**: `real-project-1` 是交付链路验证样板，用于证明"monorepo 内开发 → 独立构建 → 独立交付"链路可行。它**不交付给外部团队**——真正的外部生产交付需在 Phase 5（Figma 设计对接）完成、provisional 视觉替换为真实设计后，才有条件进行。
...
## Phase 5: Figma 设计对接（按需插入，长期滚动）

**状态**: 进行中

**目标**: 当用户提供 Figma 设计稿后，将 provisional token 和组件视觉替换为真实设计，并在组件持续新增/迭代过程中滚动补齐设计系统 token。
...
- 本 Phase 可在 Phase 1-4 任意阶段插入，取决于 Figma 设计稿何时就绪。
- 本 Phase 不是一次性收口任务，而是可跨对话、跨组件长期滚动推进的工作面。
- 当前已处于进行中，但尚未整体准出；现状是“基础全局样式 token 已落地首轮，`Button` 的 Base 类当前范围、`Tag` 当前范围已可冻结，`Icon`、`Form` 展示类与 `NavBar` 已落地首轮，其余组件按批次逐步推进”。
- 本 Phase 的状态词口径约定为：阶段层只使用“进行中 / 已完成”；组件批次层优先使用“已落地首轮 / 收尾中 / 可冻结”。
