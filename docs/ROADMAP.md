...
**准出标准**:
- [x] Taro 4.x 引入，packages/components 可编译 Taro 组件
- [x] Button 组件实现（provisional 视觉）+ TypeScript 类型合同 + YAML 元数据
- [x] 组件文件结构模式确立：`*.tsx / *.types.ts / *.contract.yaml / *.test.tsx / *.harness.tsx`
- [x] apps/stage 作为 Taro H5 应用可运行，展示 Button 所有变体和状态
- [x] stage shell 组件与样式隔离验证通过（`__stage-` 命名空间 + import 边界）
- [x] scripts/validate-boundaries.ts 通过
- [x] 执行计划文档产出：docs/exec-plans/active/phase-1.md
